...
@token_required
def create_user():
    """Fixed implementation of the create_user endpoint to handle 422 errors"""
    ...
    try:
        ...
        print(f"Admin create_user: Received data: {data}")
        ...
        try:
            ...
            """, (
                ...
                data['last_name'],
                data['role'],
                data['password']  # Plaintext for development
            ))
            ...
            if data['role'] == 'mentor':
                ...
                """, (
                    ...
                    mentor_data.get('designation', ''),
                    mentor_data.get('max_students', 5)
                ))
                mentor_id = cursor.lastrowid
                print(f"Created mentor with ID: {mentor_id}")
                
            elif data['role'] == 'student':
                student_data = data.get('student_info', {})
                full_name = f"{data['first_name']} {data['last_name']}"
                reg_number = student_data.get('registration_number', '')
                
                if not reg_number:
                    return jsonify({'error': 'Registration number is required for student profile'}), 400
                ...
                # Assign to mentor if mentor_id provided
                mentor_id = student_data.get('mentor_id')
                if mentor_id:
                    cursor.execute("""
                    ...
                    # Also update the student record with the mentor_id
                    cursor.execute("""
                        UPDATE students SET mentor_id = %s WHERE id = %s
                    """, (mentor_id, student_id))
            
            # Commit the transaction
            conn.commit()
            print("Admin create_user: Transaction committed successfully")
            ...
            conn.rollback()
            print(f"Admin create_user: Database error: {str(db_error)}")
            return jsonify({"error": f"Database error: {str(db_error)}"}), 500
        finally:
            conn.close()
            
    except Exception as e:
        print(f"Admin create_user: Error creating user: {str(e)}")
        import traceback
        traceback.print_exc()
...
def reset_password(user_id):
    try:
        ...
        # Store plaintext password directly instead of hashing it
        user.password = data['new_password']
        db.session.commit()
        
        print(f"Password reset successful for user {user_id} ({user.email}) to plaintext password")
...
@bp.route('/direct-reset-password/<int:user_id>', methods=['POST'])
@admin_required
def direct_reset_password(user_id):
    """Reset password using direct SQL queries to bypass any ORM issues"""
    try:
        data = request.get_json()
        if 'new_password' not in data:
            return jsonify({'error': 'New password is required'}), 400
        ...
        # Get the plain text password (no hashing)
        new_password = data['new_password']
        
        print(f"Resetting password for user ID {user_id} ({user.email}) to new plain password")
        
        # Use direct SQL to update the password
        conn = None
        try:
            conn = pymysql.connect(
                host='localhost',
            ...
            with conn.cursor() as cursor:
                # Update password directly in the database without hashing
                cursor.execute(
                ...
                if affected_rows == 0:
                    return jsonify({'error': 'Password update failed - no rows affected'}), 500
                    
                # Log the password change
                admin_id = request.token_data.get('user_id')
                try:
                    cursor.execute(
                        "INSERT INTO password_change_logs (user_id, changed_by_id, changed_at) VALUES (%s, %s, NOW())",
                        (user_id, admin_id)
                    )
                except Exception as log_error:
                    print(f"Warning: Could not log password change: {str(log_error)}")
                
            # Commit the transaction
            conn.commit()
            
            # Verify the change by querying the database again
            with conn.cursor() as cursor:
                cursor.execute("SELECT password FROM users WHERE id = %s", (user_id,))
                result = cursor.fetchone()
                if result:
                    updated_password = result[0]
                    print(f"Verified new password in database for user {user_id}")
                    
                    # Double check: Update the User object in the session to match the database
                    if hasattr(user, 'password') and user.password != updated_password:
            ...
            import traceback
            traceback.print_exc()
            return jsonify({'error': f'Database error: {str(sql_error)}'}), 500
        finally:
            if conn:
                conn.close()
